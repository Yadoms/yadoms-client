...
export function createRestServerServiceFactory(http: HttpClient, router: Router, location: Location, errorService: ErrorService) {
  return new RestServerService("http://localhost:8080", http, router, location, errorService);
}
